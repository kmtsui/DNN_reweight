...
namespace cppflow {
    ...
    public:
        ...
        tensor operator()(const tensor& input, std::string input_string, std::string output_string);
...
namespace cppflow {
    ...
    inline std::vector<tensor> model::operator()(std::vector<std::tuple<std::string, tensor>> inputs, std::vector<std::string> outputs) {
        ...
        for (int i=0; i<inputs.size(); i++) {
            ...
            // Operations
            const auto tmp = parse_name(std::get<0>(inputs[i]));
            const auto op_name = std::get<0>(tmp);
            const auto op_idx = std::get<1>(tmp);
        ...
        std::unique_ptr<TF_Tensor*[]> out_val(new TF_Tensor*[outputs.size()]);
        for (int i=0; i<outputs.size(); i++) {

            const auto tmp = parse_name(outputs[i]);
            const auto op_name = std::get<0>(tmp);
            const auto op_idx = std::get<1>(tmp);
    ...
    inline tensor model::operator()(const tensor& input) {
        std::tuple<std::string, tensor> input_tuple ("serving_default_input_1", input);
        std::vector<std::tuple<std::string, tensor>> inputs;
        inputs.emplace_back(input_tuple);
        std::vector<std::string> outputs;
        outputs.emplace_back("StatefulPartitionedCall");
        //return (*this)({{"serving_default_input_1", input}}, {"StatefulPartitionedCall"})[0];
        return (*this)(inputs,outputs)[0];
    }

    inline tensor model::operator()(const tensor& input, std::string input_string, std::string output_string ) {
        std::tuple<std::string, tensor> input_tuple (input_string, input);
        std::vector<std::tuple<std::string, tensor>> inputs{input_tuple};
        std::vector<std::string> outputs{output_string};
        return (*this)(inputs,outputs)[0];
    }
}
